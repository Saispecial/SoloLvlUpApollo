...
const realmIcons: Record<Realm, React.ReactNode> = {
  "Self-Awareness & Recognition": <Brain className="w-5 h-5" />,
  "Emotional Regulation": <Heart className="w-5 h-5" />,
  "Empathy & Patient Care": <Users className="w-5 h-5" />,
  "Team Communication": <Users className="w-5 h-5" />,
  "Stress Management": <Dumbbell className="w-5 h-5" />,
}

const realmColors: Record<Realm, string> = {
  "Self-Awareness & Recognition": "from-blue-400 to-cyan-500",
  "Emotional Regulation": "from-pink-400 to-purple-500",
  "Empathy & Patient Care": "from-green-400 to-emerald-500",
  "Team Communication": "from-yellow-400 to-orange-500",
  "Stress Management": "from-red-400 to-pink-500",
}

const realmColorClasses: Record<Realm, string> = {
  "Self-Awareness & Recognition": "text-blue-400",
  "Emotional Regulation": "text-pink-400",
  "Empathy & Patient Care": "text-green-400",
  "Team Communication": "text-yellow-400",
  "Stress Management": "text-red-400",
}
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          title="Daily Average"
          value={(performanceMetrics.dailyAverage?.modulesCompleted ?? 0).toFixed(1)}
          subtitle="Modules/Day"
          icon={<Target className="w-6 h-6" />}
          color="from-blue-400 to-cyan-500"
        />
        <MetricCard
          title="Weekly EI Points"
          value={weeklyStats?.totalEIPoints ?? 0}
        ...
        <MetricCard
          title="Average Mood"
          value={(weeklyStats?.averageMood ?? 0).toFixed(1)}
          subtitle="/10"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-mono text-green-400">
              {(performanceMetrics.dailyAverage?.modulesCompleted ?? 0).toFixed(1)}
            </span>
          </div>
          <FuturisticProgressBar
            value={(performanceMetrics.dailyAverage?.modulesCompleted ?? 0) * 10}
            maxValue={100}
            color="cyan"
            height="sm"
          />
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-400">EI Points Earned</span>
            <span className="text-sm font-mono text-green-400">
              {(performanceMetrics.dailyAverage?.eiPointsEarned ?? 0).toFixed(1)}
            </span>
          </div>
          <FuturisticProgressBar
            value={(performanceMetrics.dailyAverage?.eiPointsEarned ?? 0) / 10}
            maxValue={100}
            color="yellow"
            height="sm"
          />
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-mono text-green-400">{(weeklyStats?.averageMood ?? 0).toFixed(1)}</span>
          </div>
          <div className="flex justify-between items-center">
